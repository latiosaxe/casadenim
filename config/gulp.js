...
let devProcesses = gulp.series(
	clean,
	gulp.parallel(styleHtml, scripts, pages, copy, vendor, images, watch, reload)
);

if (argv.shopify === 'true') {
	devProcesses = gulp.series(
		clean,
		shopifyWatch,
		gulp.parallel(styleShopify, scripts, pages, copy, vendor, images, watch, reload)
	);
}
...
gulp.task(
	'buildShopify',
	gulp.series(clean, styleShopify, scripts, pages, copy, vendor, images, optimize)
);
